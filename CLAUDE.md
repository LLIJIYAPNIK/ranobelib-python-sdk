...
21. **`CHANGELOG.md` с автогенерацией через GitHub Actions.** Формат — [Keep a
    ...
    - Уже вышедшие версии (`v0.1.0`, `v0.2.0`, теги и GitHub Releases существуют на момент
      ...
      уже в этом PR файл покрывает обе версии, а не только будущие релизы. `v0.3.0` вышел
      позже (см. выше про `ci:`/`Changed`) и `changelog.yml` его пропустил — файл пришлось
      руками пересобрать тем же `git-cliff` уже с исправленным `cliff.toml`, тем же способом
      локального прогона (см. предыдущий пункт про `GITHUB_REPO`), а не написать вручную.
