...
докстринг `_ImageSrcRewriter`. Это единственный экспортёр, у которого `export()` реально
делает сетевые запросы — отсюда и исправление сигнатуры `Exporter.export()` на `async`
(см. выше).
