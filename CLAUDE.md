...
- Логика выбора branch/team по умолчанию, когда `branch_id` не передан — не "первая ветка
  в списке" (проверено на тайтле с двумя переводами, см. `docs/api-notes.md`). Нужно перед
  реализацией `MultipleTranslationsError`/дефолта перевода (пункт 9 дорожной карты).
